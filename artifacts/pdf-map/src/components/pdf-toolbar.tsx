...
function ZoomControls() {
  ...
  return (
    <>
      <Button variant="ghost" size="icon" onClick={() => zoomOut()} className="h-8 w-8 hover:text-primary">
        <ZoomOut className="h-4 w-4" />
      </Button>
      <Button variant="ghost" size="icon" onClick={() => resetTransform()} className="h-8 w-8 hover:text-primary">
        <Maximize className="h-4 w-4" />
      </Button>
      <Button variant="ghost" size="icon" onClick={() => zoomIn()} className="h-8 w-8 hover:text-primary">
...
export function PdfToolbar({ columns, setColumns, pageCount, fileCount, onUploadClick }: PdfToolbarProps) {
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 flex items-center h-12 px-2 bg-card/80 backdrop-blur-md border border-border rounded-full shadow-lg shadow-black/20">
      ...
      {/* Actions */}
      <div className="px-2">
        <Button variant="ghost" size="sm" onClick={onUploadClick} className="h-8 gap-2 hover:text-primary">
